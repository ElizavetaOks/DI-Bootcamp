...
SELECT c.first_name, cp.isLoggedIn FROM customer AS c
LEFT JOIN customer_profile AS cp
ON c.id = cp.customer_id

-- The number of customers that are not LoggedIn
SELECT count(c.first_name) FROM customer AS c
LEFT JOIN customer_profile AS cp
ON c.id = cp.customer_id
...
SELECT AVG(s.age) AS average_age FROM Library AS l
JOIN student AS s ON l.student_id = s.student_id
JOIN book AS b ON l.book_fk_id = b.book_id
WHERE b.title = 'Alice In Wonderland';

-- Delete a student from the Student table, what happened in the junction table ?
DELETE FROM Student WHERE student_id = 1; -- all records about this student will be deleted automatically from the junction table 
